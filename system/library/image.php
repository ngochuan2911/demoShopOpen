...
class Image {
	...
	public function resize_and_crop($width, $height, $type = "") {
		if(!$this->width || !$this->height) {
			return;
		}
		...
		if($scale_w > $scale_h) {
			$src_x = 0;
			$src_y = ($this->height - $height / $scale_w) / 2;

			$src_w = $this->width;
			$src_h = round($height / $scale_w);
		} else {
			$src_x = ($this->width - $width / $scale_h) / 2;
			$src_y = 0;
		...
		if($type == 'top') {
			imagecopyresampled($this->image, $image_old, 0, 0, 0, 0, $width, $height, $src_w, $src_h);
		} else {
