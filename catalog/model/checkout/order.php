...
class ModelCheckoutOrder extends Model {
	public function addOrderSmall($data){
		$total = 0;
		if(isset($data['products']) && !empty($data['products'])) foreach($data['products'] as $product){
			$total += $product['total'];
		}

		$this->db->query("INSERT INTO `" . DB_PREFIX . "order` SET total = '" . (float)$total . "', firstname = '" . $this->db->escape($data['firstname']) . "', lastname = '" . $this->db->escape($data['lastname']) . "', payment_method = '" . $this->db->escape($data['payment_method']) . "', store_id = '" . $this->db->escape($data['store_id']) . "', store_url = '" . $this->db->escape($data['store_url']) . "', store_name = '" . $this->db->escape($data['store_name']) . "', currency_code = '" . $this->db->escape($data['currency_code']) . "', note = '" . $this->db->escape($data['note']) . "', email = '" . $this->db->escape($data['email']) . "', telephone = '" . $this->db->escape($data['telephone']) . "', customer_group_id = '" . (int)$data['customer_group_id'] . "',customer_id = '" . (int)$data['customer_id'] . "', payment_address_1 = '" . $this->db->escape($data['payment_address_1']) . "', date_added = NOW(), date_modified = NOW()");
