...
class ControllerSaleOrder extends Controller {
	...
	public function getForm() {
		...
		if (!empty($order_info)) {
			...
			$data['order_id'] = 0;
			$data['store_id'] = $this->request->server['HTTPS'] ? HTTPS_CATALOG : HTTP_CATALOG;
	...
	public function info() {
		...
		if ($order_info) {
			...
			$data['note'] = $this->model_sale_order->getOrderNote($order_id);
		...
		foreach ($orders as $order_id) {
			...
			if ($order_info) {
				...
				if ($store_info) {
					...
					$store_address = $store_info['config_address'][$this->config->get('config_language_id')];
					$store_email = $store_info['config_email'];
					$store_telephone = $store_info['config_telephone'];
					$store_fax = $store_info['config_fax'];
				} else {
					$config_address = $this->config->get('config_address');
					$store_address = $config_address[$this->config->get('config_language_id')];
