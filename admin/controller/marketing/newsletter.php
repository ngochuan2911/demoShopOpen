...
class ControllerMarketingNewsletter extends Controller
{
	...
	{
		$this->language->load('marketing/newsletter');
	...
	public function delete()
	{
		...
		if(isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $newsletters_id) {
				$this->model_marketing_newsletter->deleteNewsletter($newsletters_id);
			}
	...
	protected function getList()
	{
		...
		$results = $this->model_marketing_newsletter->getNewsletters($data);
		...
		foreach ($results as $result) {
			...
			$action[] = array(
				...
				'href' => $this->url->link('marketing/newsletter/view', 'token=' . $this->session->data['token'] . '&newsletters_id=' . $result['newsletters_id'] . $url, 'SSL')
			);
			$data['newsletters'][] = array(
		...
		$data['sort_name'] = $this->url->link('marketing/newsletter', 'token=' . $this->session->data['token'] . '&sort=name' . $url, 'SSL');
		$data['sort_email'] = $this->url->link('marketing/newsletter', 'token=' . $this->session->data['token'] . '&sort=email' . $url, 'SSL');
		$data['sort_date_added'] = $this->url->link('marketing/newsletter', 'token=' . $this->session->data['token'] . '&sort=date_added' . $url, 'SSL');
		...
		$pagination->url = $this->url->link('catalog/newsletter', 'token=' . $this->session->data['token'] . $url . '&page={page}', true);
		...
		$data['deletel'] = $this->url->link('marketing/newsletter/delete', 'token=' . $this->session->data['token'] . $url, 'SSL');
